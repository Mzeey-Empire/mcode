...
export const PlanQuestionOptionSchema = lazySchema(() =>
  z.object({
...
/** A single selectable option within a plan question. */
export type PlanQuestionOption = z.infer<ReturnType<typeof PlanQuestionOptionSchema>>;

/** A single clarifying question proposed by the model in plan mode. */
export const PlanQuestionSchema = lazySchema(() =>
  z.object({
...
/** A single clarifying question proposed by the model in plan mode. */
export type PlanQuestion = z.infer<ReturnType<typeof PlanQuestionSchema>>;

/** A user's answer to a single plan question. */
export const PlanAnswerSchema = lazySchema(() =>
  z.object({
...
/** A user's answer to a single plan question. */
export type PlanAnswer = z.infer<ReturnType<typeof PlanAnswerSchema>>;

/** Batch of plan questions emitted by the model. */
export const PlanQuestionBatchSchema = lazySchema(() =>
  z.object({
    threadId: z.string(),
...
/** Batch of plan questions emitted by the model. */
export type PlanQuestionBatch = z.infer<ReturnType<typeof PlanQuestionBatchSchema>>;
