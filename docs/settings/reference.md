...
| `model.defaults.reasoning` | enum | `"high"` | `"none"` \| `"minimal"` \| `"low"` \| `"medium"` \| `"high"` \| `"xhigh"` \| `"max"` \| `"ultrathink"` | - | Default reasoning effort level. Tiers in ascending order: `low < medium < high < xhigh < max`. `"none"` and `"minimal"` map to OpenAI Codex effort presets; Claude models normalize them to `"low"`. `"xhigh"` requires Opus 4.8 or Opus 4.7 for Claude (also valid for Codex models). `"max"` requires Opus 4.8, Opus 4.7, Opus 4.6, or Sonnet 4.6; it normalizes to `"high"` at runtime on other Claude models. `"ultrathink"` is a virtual top tier (Opus 4.8/4.7/4.6 and Sonnet 4.6) that prepends `"Ultrathink:\n"` to the user prompt and sends `"max"` effort to the SDK. Haiku 4.5 ignores this setting entirely -- the effort parameter is not sent for that model. |
